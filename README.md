...
実際の絵の具をパレットへ置き、重ねて混ぜ、水で薄め、その配合を保存できる日本語Webアプリです。選んだ色は長押ししたまま動かして波状に伸ばせます。作った色は、そのまま「おえかき」と「ぬりえ」で使えます。
...
パレットへ置いた各絵の具は1単位のまま記録されますが、表示上は中心から縁へなめらかに薄くなる局所的な重みを持ちます。絵の具同士が重なった場所では、その地点へ届く各材料の重みから顔料比率を更新します。長押し後に動かした軌跡には選択中の絵の具または保存レシピ色だけを追加し、保存色は元の配合比率を保ちます。「すべて混ぜる」では、操作時点の水を含むレシピ全体をなじませます。
