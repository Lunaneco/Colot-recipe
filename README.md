...
- RITの実測アーティスト絵の具から導出されたK・S（380–750 nm、38波長）と二定数Kubelka–Munkモデルによる減法混色
...
- おえかき・ぬりえでは、現在色・色名・「色を変える」を一つの大きなボタンで常時表示し、保存パレットの選択色もチェックと「選択中」で表示
...
`lib/paintCalibration.ts` は、RIT Artist Paint Spectral Databaseの実測塗膜から導出されたGolden Heavy Body Acrylicの吸収係数 `K(λ)` と散乱係数 `S(λ)` を固定プロファイルとして使用します。収録値は380〜750 nm、10 nm間隔の38点です。数値転記元とライセンスは [`THIRD_PARTY_NOTICES.md`](./THIRD_PARTY_NOTICES.md)、調査根拠と次の測定計画は [`docs/PAINT_MIXING_RESEARCH.md`](./docs/PAINT_MIXING_RESEARCH.md)、調べた色見本・測色データの採否は [`docs/PAINT_DATA_SOURCES.md`](./docs/PAINT_DATA_SOURCES.md) に記録しています。

画面色、CC0作品、公開分光資料の使い分けと、今回の色変更UIへ反映した知見は [`docs/OPEN_COLOR_RESEARCH.md`](./docs/OPEN_COLOR_RESEARCH.md) に記録しています。

| アプリの材料 | 測定塗料 | Colour Index |
| --- | --- | --- |
| 赤 | Pyrrole Red | PR254 |
| 青 | Cerulean Blue Chromium | PB36 |
| 黄 | Hansa Yellow Opaque | PY74 |
| 黒 | Bone Black | PBk9 |
| 白 | Titanium White | PW6 |

1タップは「完成絵の具の相対的1部」です。赤2・青1なら、各波長の赤の `K` と `S` をそれぞれ青の2倍加えます。密度や乾燥顔料体積分率まで測定していないため、アプリ単位をグラムや乾燥顔料質量とは表現しません。

1. Duncanの理想混合近似により、各波長で `Kmix(λ) = Σ cᵢKᵢ(λ) / Σ cᵢ`、`Smix(λ) = Σ cᵢSᵢ(λ) / Σ cᵢ` を局所の連続比率のまま計算
2. `q = Kmix / Smix` とし、桁落ちしにくい `R∞ = 1 / (1 + q + √(q² + 2q))` で不透明層の内部反射率へ変換
3. RITのSPIN原測定とは区別し、Wacton互換のSPEX表示ジオメトリというレンダリング仮定のもと、Saunderson係数 `k1 = 0.03`、`k2 = 0.65` で表面反射を補正
4. CIE公式の1931 2°等色関数とD65でXYZへ直接積分し、同じD65のsRGBへ変換（観測者差へBradford変換を誤用しない）
5. 指定済み純色の表示契約と、実測スペクトルの混色残差をOKLabで連続的に両立。白ティントは分光色相を保ったまま二定数スペクトルのD65輝度進行へ合わせ、最終8bit RGBの見える丸め逆転を局所補正
6. sRGB色域外はOKLabの明度と色相を保ち、彩度だけを縮めてHEX/RGB/HSLへ出力

旧モデルの「全顔料の散乱力が等しい」という単一定数仮定、理想白、混ざる色数によって物理効果を弱める経験係数は使用しません。`1:1`と`2:2`は同じ色になり、材料の投入順やオブジェクト順でも変わりません。描画用キャッシュは色比率だけを再利用し、透明度、水分比、粘度は画素ごとの実量から必ず再計算するため、薄い縁を過大に濃く表示しません。

指定済みの開始色（赤 `#E60012`、青 `#00A1E9`、黄 `#FFF100`、黒 `#000000`、白 `#f8f3e8`）は表示上の契約として維持します。HEXと実測顔料は別の色定義なので、画面色では配合比に応じた端点差をOKLabで加えながら分光相互作用を残します。白ティントは分光計算後の色相・彩度を保持し、輝度だけを二定数モデルが予測したD65の進み方へ合わせます。最後の8bit化は輝度補正後の分光色から各チャンネル最大4段以内の局所候補だけを比較し、単独チャンネルの丸めによる見える明度逆転を抑えます。底層の混合反射率は全比率で二定数計算を保ち、表示校正と分離してテストできます。

水は顔料のKまたはSへ足しません。固有色を保ったまま透明度、濃さ、粘度、広がり、乾燥速度へ作用するアプリ内近似です。広がった顔料は面積に応じて質量補正し、中心の1単位を保ちます。白は実測由来のTitanium Whiteとして強い散乱と有限の吸収を持ち、配合比どおりにティントへ作用します。

### 再現範囲と限界

「赤」「青」「黄」やHEXだけでは実物の絵の具を一意に決められません。同じ表示色でも異なる反射スペクトルを持つため、別メーカー、別顔料、製造ロット、下地、膜厚、乾燥状態、照明が変われば結果も変わります。本アプリの結果は上表の固定プロファイルを基準にした予測であり、すべての市販絵の具に共通する絶対値ではありません。現在は二定数でも不透明・光学的無限厚の近似であり、有限膜厚と紙の反射、濃度による粒子凝集、顔料体積濃度の非線形性までは再現しません。また3つの有彩顔料だけでは、実物の全顔料色域を再現できません。したがって「KとSを分けて配合比を計算する」ことは検証していますが、実物との色差ゼロは主張しません。
...
「レシピ保存」で書き出すファイルは、版番号と保存色配列を持つ次の形です。下の例は、赤3・黄2・白1・水2を置いて「すべて混ぜる」を実行した `#E2573F` の実データに合わせています。
...
lib/
  cieD65.ts           CIE 1931 2°等色関数と標準イルミナントD65
  paintCalibration.ts 実測由来の顔料K・Sと校正条件
  colorScience.ts     二定数Kubelka–Munk混色と画面色変換
...
docs/PAINT_DATA_SOURCES.md 色見本・測色データの採否と独立検証結果
...
- 顔料モデルはGolden Heavy Body Acrylicの特定測定試料を基準にした近似で、別メーカー、別ロット、膜厚、下地、濡れ・乾燥状態まで共通に再現するものではありません。
